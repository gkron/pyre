...
def extract_Experience(text):
    #temp = re.search(r'(\d+) year', text) 
    frac = re.findall(r'\d*\.?\d+ year', text) 
    f1= str(frac).split(" ")
    f2=str(f1[0])+ " "+str(f1[1])
   ## print(f2)
    #frac2 = re.findall(r'\d*\.?\d+ months', text)
    #print(frac)
    #res = list(map(int, temp))
   #print(type(temp))
    #print("The numbers list is : " ,str(frac) )
   #return (str(frac[0].split()[0].strip(';')))
    return f2
...
def extract_companyDeatils(nlp_text, noun_chunks):
    ...
    :return: list of company extracted

    '''

    tokens = [token.text for token in nlp_text if not token.is_stop]

    data = pd.read_csv(os.path.join(os.path.dirname(__file__), 'companieslist.csv')) 

    company = list(data.columns.values)
    ...
    for token in noun_chunks:
        ...
        token = token.text.casefold().strip()

        if token in company:

            companylist.append(token)

    return [i.capitalize() for i in set([i for i in companylist])]                  